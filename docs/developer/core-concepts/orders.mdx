...
When an order transitions from cart to checkout, Spree picks the [stock location](/developer/core-concepts/inventory#stock-locations) for each shipment using configurable [Order Routing](/developer/core-concepts/shipments#order-routing) rules — see Shipments for the full details.
...
- [Build Custom Order Routing](/developer/how-to/custom-order-routing) — Custom rules and strategies for fulfillment-location decisions
